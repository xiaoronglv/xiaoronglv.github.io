...
Surge 的功能并不仅限如此，它还可以对 HTTPs 进行抓包，方便调试 API。

单独购买网络调试工具 Charles Web Debugging Proxy 就要花费50美金啊，而买一个 Surge，就可以两份用途，简直赚翻了。

## Proxy Server

Surge 是一个网络软件，并不提供 Proxy 服务器，所以你需要搭建自己的 Proxy 服务器。但是不建议这么干，因为太浪费时间，时间就是金钱，所以推荐你购买这类服务商，比较稳定的是 `QWdlbnRORU8=`。
